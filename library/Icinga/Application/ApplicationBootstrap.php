...
abstract class ApplicationBootstrap
{
    ...
    protected function __construct($baseDir = null, $configDir = null, $storageDir = null)
    {
        ...
        if ($storageDir === null) {
            ...
            if ($storageDir === false) {
                $storageDir = Platform::isWindows() ? Icinga::app()->getBaseDir('storage') : '/var/lib/icingaweb2';
            }

            $realpath = realpath($storageDir);
            $storageDir = $realpath === false ? $storageDir : $realpath;
        }
